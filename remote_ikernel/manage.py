...
def get_existing_kernel_specs():
    """
    Get a dict of existing remote ipython kernelspecs

    Returns
    -------
    existing_kernels : dict
        Dict of existing remote ipython kernelspecs
        kernel_name --> kernel_spec
    """
    kernel_names = filter(
        lambda x: x.startswith(RIK_PREFIX), ks.find_kernel_specs().keys()
    )
    kernel_specs = {name: ks.get_kernel_spec(name) for name in kernel_names}
    return kernel_specs
...
def manage():
    ...
    parser.add_argument(
        "--show", "-s", nargs="*", help="Print the contents " "of the " "kernel. Leave "
    )
    parser.add_argument(
        "--add",
        "-a",
    ...
    parser.add_argument(
        "--launch-cmd",
        help="Override the command used to "
